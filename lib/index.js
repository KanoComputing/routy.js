...
Router.prototype.bindLinks = function () {
    ...
    this._clickListener = function (e) {
        var href = getHrefRec(e.target),
            ...
            downloadAttr = e.target.hasAttribute('download'),
            ignore = e.target.hasAttribute('data-routy-ignore');

        if (
            href && EXCLUDE_PATTERN.test(href) && !blankTarget &&
            !specialKey && !downloadAttr && !ignore
        ) {
            e.preventDefault();
...
Router.prototype.goTo = function (path, push) {
    ...
    if (this._html5) {
        if (this._loc !== path && location.hash.length > 1) {
            this._preventChange = true;
            location.hash = '';
        }
...
//
// Get the path from current location
//
